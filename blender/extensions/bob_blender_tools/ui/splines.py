...
coordinated bundle of effects. the first pass shipped the plumbing and the follow-terrain family
(dirt path, trail, road); the terrain overlay gives terrain shape its own standalone GN overlay:

- Terrain shape (the terrain overlay): each curve carves a bench through the terrain via its OWN curve_overlay
 modifier stacked on the terrain object (docs/SPLINES.md 4.3). One modifier per curve, so a network
 of paths composes instead of the single inline path the first pass used. The overlay reads the
 incoming terrain geometry, so it works on any terrain mesh, and writes the curve mask attributes
 (bbt_curve_mask / bbt_curve_dist) that the shader and scatter READ, so a downstream effect never
 re-solves proximity or duplicates a knob (docs/SPLINES.md 9 #2 / #4).
- Terrain material (the material band): a surface band along the curve. Build configures a terrain-material
 layer keyed to the overlay's bbt_curve_mask (materials.apply_curve_surface, the same shape as the
 Flow mask keying a riverbed layer), so a road/dirt surface reads only along the path with no
 re-solved proximity. One shared curve-surface layer for now (all paths share it); distinct per-role
 surfaces need distinct mask attributes, a later step.
- Scatter (the scatter mask): a curve drives scatter through the baked bbt_curve_mask, not a scn.path proximity.
 ...
 scatter_along recipe to place instances ALONG a curve (fence posts), optionally aligned. The Paths
 Scatter channel flips unbound layers to clear and rebuilds; per-layer modes live in Scatter.
...
Depth) live ONCE on the curve's overlay modifier and snapshot-restore across a rebuild like any GN
knob, so nothing here duplicates a width/depth knob. The list binds each row to its curve by
PointerProperty (section 9 #10), not by name, so it survives renames.

At Build the curve is draped onto the terrain (when the terrain carries a baked heightmap) so its Z
follows the ground; without a heightmap the curve's authored Z is used. Cross-section profiles that
make road/trail structurally distinct (a road bench + shoulders, embankments) and the side/tangent
field for them arrive with their consumers; the terrain overlay roles differ by knob defaults and
...
# guard (owned by core) suppresses the per-prop sync while seed_role_params sets a whole role at
# once.
