# Generation: measured baselines

Every number the generation integration rests on, grouped by what was measured, plus the review
findings that turned into rules and the decisions that are answered. [GENERATION.md](GENERATION.md)
is the reference you read while working; this file is what you read when a number is questioned.

Each section below names **what was measured**, in the order the measurements were made, because a
later baseline is sometimes a correction of an earlier one and the order is what makes that legible.
The sections carry their own dates where the date matters.

**This is a compression, not the original.** Each measurement round was 150 to 330 lines of
long-form prose with its corrections spelled out; what is kept is the scope, the verdict, the
decisive numbers and the gate that proved them. The full text was last present in
`docs/COMFYUI.md` at commit `8e7c4a6` and is recoverable from git history:
...
## The texture-set sampler

**Texture-set sampler in Shaders (the BobShaders map-socket work), plus the generated pack root.**
...
## The first spike: prompt to a rendered terrain layer
...
## Variants, preflight and derived maps
...
## Prompt to a scattered asset

**The mesh-generation family and B together: `mesh_subject`, `mesh_geom_trellis`, `mesh_texture`, `mesh_simplify_uv`, plus `mesh_geom` Hunyuan as the plumbing smoke test.
...
Partials, both carried to the route A/B: open surfaces needed `remesh: off` to appear at all, and the opacity
channel did not reach the finished material. Foliage measured three ways on three prompts, which is
the origin of the foliage rule in GENERATION.md: a flat leaf gave 11,610 boundary edges at a 0.0422
thin ratio, a fern frond 51,842 edges as a bushy volume, and a broadleaf sprig **15 edges**, i.e. a
closed blob.

## One-shot against the staged chain
...
shared `mesh_subject` subject each. Plus the opacity channel, the asset gate's named partial.**
...
## Stylise, paint and multi-view conditioning

**The look-dev stylise family then B-stylised, in that order because they share a graph: `stylize_render` and `stylize_render_est` stylise (render
...
**The macro-heightmap family: `heightmap_macro`, `comfy_maps.macro_field`, the `macro` op and `params.with_macro`, and Generate Base
in the Terrain panel.**

Passed, and it answered the bit-depth floor in the negative. A prompted silhouette survives an erosion pass at
...
**the bit-depth floor's terracing does not occur at 8 bits, and does not occur at 5 either** (histogram concentration
...
(the slope-area sign, still open in GENERATION.md).
...
at seam 8.466 and `heightmap_macro`'s open route at 10.086, i.e. verified untiled. Still open upstream as the staged-copy fault.
...
- **Block-out: Hunyuan through Omni and `mesh_geom_ctrl`**, decided by the control gate. `mesh_geom_alt` takes no control, so the grid has
  no cell for it.

Both models 10/10 inside the 4,000 budget, worst UV overlap 0.00047, three of ten carried through
steps 6 to 8 on both models with every the asset gate asset check passing.
...
**The dense-mesh question, answered both ways.** The control gate's alignment fix raises transferred detail on 4 of 4 assets (14x on
the fern), and the dense mesh still buys nothing at this budget (1 of 4 better than the control, 2 of
4 worse).

## Omni bounding-box control

**the control-ordering question's first half: `mesh_geom_bbox` (`mesh_geom_bbox`, Omni's bounding-box control), the control mode as a value
...
Passed, and the control-ordering question's answer is no. Eight corners do not replace 8,192 points: footprint IoU **0.5766**
...
this from the control gate's silently-random wrapper: Bob's proportions beat the node's `auto_bbox` guess 3 of 3
...
finished asset through `mesh_geom_bbox`, `mesh_simplify_uv`, `mesh_texture` and steps 6 to 8 passes every the asset gate check (3,902 faces, UV overlap
...
## Omni voxel control, and the control ordering

**the control-ordering question's remainder and its close: `mesh_geom_voxel` (`mesh_geom_voxel`, Omni's voxel control),
...
Passed, and it closed the control-ordering question without changing the default. An occupancy grid reaches footprint IoU
...
## Rules the review findings left behind

The first design had real defects. Each is kept as the finding, the rule it turned into, and where
that rule now lives, because these rules are most of what the integration became.

| Rule | What the review found | The rule it became |
|---|---|---|
| **the vertical-spike correction** | Two rounds of plumbing planned before anything an artist could see. | Build a thin vertical spike first and generalise the plumbing afterwards, against something that already worked. |
| **the UV-ownership rule** | Generated geometry has no UVs at all, and the whole material story assumed it did. | UV creation is a mandatory Blender-owned stage; the pipeline order is pinned. |
| **the division of labour** | Retopology, UV and baking pointed at the wrong side of the fence. | Division of labour: **ComfyUI generates; Blender retopologises, unwraps, bakes, LODs, packs.** ML retopo is a nice-to-have, not a dependency. |
| **the paint-order correction** | Paint-versus-retopo ordering undefined, and one order destroys the textures. | Paint the dense mesh, bake dense to low. **Superseded by the projection-route finding**, then by the open-surface rule deleting the wrapper. |
...
| **the title-template rule** | Title-based templating unguarded; API export drops nodes feeding no output. | Preflight validates every `BOB_*` title is present and unique. |
| **the wall-clock budget rule** | No time budgets, so no way to tell a slow path from a broken one. | Every gate carries a measured wall-clock target. |
| **the sampler-budget rule** | The sampler's node cost was underestimated: six layers times five maps is thirty image nodes. | One shared sampler group per layer, nodes only for enabled layers, node count and EEVEE compile in the gate. |
| **the threading rule** | Threading details missing; a job outliving a file load corrupts state. | Job registry clears on `load_post`; every `bpy` touch happens in the timer tick. |
| **the naming rule** | No naming or dedup rule, so a second "rock" overwrites the first. | Slugged unique names, never an implicit overwrite. |
| **the derivation rule** | "Author the workflows from scratch" was wrong; ten official Hunyuan3D templates already ship. | Bob's graphs are **derived** from templates: convert to API format, retitle `BOB_*`, record upstream name and version. Upstream changes become a diff. |
| **the local-only rule** | The retopology question had an answer in the repo, and it is cloud-only (`TencentSmartTopologyNode` and four others, $0.20 to $1.00, comfy.org auth). | Evaluated and **rejected** on the local-only decision. Recorded so it is not rediscovered as new. |
| **the retopology tier rule** | Local-only makes Blender the only retopo path and the paint wrapper the only Hunyuan PBR route. | Retopology tiered on the observation that **quad topology barely matters for static scatter props**; Quadriflow reserved for hero. Paint priority inverted to `mesh_paint_views`. |
| **the projection-route finding** | The paint wrapper is the same algorithm with a worse rasteriser; Blender already rasterises and projects in-process. | Blender-side route promoted. **Confirmed by the stylise gate** in 300 lines of numpy: 92.6% of chart texels from eight views, 29 texels left for hole fill. What it loses is cross-view consistency (22.3 to 26.5 of 255 adjacent, 30.1 front-to-back), which is what MV-Adapter would have fixed, and MV-Adapter was declined. |
| **the open-surface rule** | TRELLIS.2 changes the geometry and paint decision, and one reason dominates: O-Voxel represents **open surfaces**, which Hunyuan's watertight SDF cannot. | TRELLIS.2 primary (MIT, native PBR); the compiled-rasteriser Hunyuan paint wrapper dropped; `mesh_paint_views` demoted to style-control alternative; `Trellis2Simplify` and `Trellis2UVUnwrap` became an A/B against Blender rather than an assumption. Hunyuan keeps multi-view conditioning, Omni control, and zero-install smoke testing. |

---

## Decisions, answered

- **The tiling-model rule: SDXL, with `RealVisXL_V5.0_fp16`** (6.9 GB, OpenRAIL++).
...
- **Further challengers (Direct3D-S2, Hi3DGen): no, and the condition is what answers it.** Adding
  one was conditional on TRELLIS.2 and Hunyuan landing close. They did not, in either direction:
  the stylise gate split them by capability and the geometry A/B split them the same way but wider. What stays is the **slot** (`mesh_geom_alt`
  plus `mesh_process`), so a future challenger costs one graph and no rewrite. Reopen with a specific gap, not
  for coverage.
- **The retopology route.** Official retopology is cloud-only, so Blender does it:
  Decimate-collapse for scatter-grade, Quadriflow for hero, tiered by intent (the retopology tier rule). Open ML
...
- **Seamless tiling: circular padding, both halves.** `ComfyUI-seamless-tiling`
...
- **Where the texture-set sampler belongs: here.** It shipped before any ComfyUI code existed, which
  is what let the first spike be a spike rather than a spike plus a sampler.
- **Geometry ambition, answered in the direction the design did not expect: scatter-grade only,
  because there is no local hero tier to have.** Quadriflow refuses every generated mesh
...
- **A cloud tier: no.** Fully local, enforced by the `api_node` preflight check rather than by
  intent.
- **Paint route priority, settled by the open-surface rule.** `Trellis2TextureMesh` is the route;
...
- **Two models or one: both, split by capability.** The pack install weakened one leg
  (`Trellis2MultiViewImageToShape` exists, so multi-view is not Hunyuan-only and that comparison
  moved to the geometry A/B). Omni block-out control and zero-install smoke testing are what Hunyuan still
  uniquely provides.
- **MV-Adapter: no, and this is the decision that closed the design out.** The case was
  ...
  **unblocks nothing** and no phase since has needed it. The defect it fixes is **below the tolerance
  ...
  never errors**: the black albedo, Omni's random projection, `auto_bbox`, `apply_input_rotation`,
  four for four), so it would have to budget for a swapped-reference null of
...
- **The dense-mesh question, re-measured through a fixed bake: it still buys nothing, and the
  route A/B's own measurement really was wrong.** Applying `comfy.stage_exports` raises transferred high-frequency
  content on 4 of 4 of the route A/B's own assets (14x on the fern, 0.02143 against 0.00153), so the route A/B's staged
  ...
  a few-hundred-face budget or a hero tier; the geometry A/B gate's part D re-scores it from cache with no GPU.
- **Which Omni control a block-out should use, answered over two gates: the point cloud, with the
  whole ordering measured rather than the winner alone.** Footprint IoU
...
Still open, and therefore kept in [GENERATION.md](GENERATION.md#open-questions): the **slope-area
gradient sign**, which belongs to the terrain engine; the **staged-copy fault**, the upstream
`comfy_aimdo` segfault, with its version tripwire in two gates; the **VRAM-handback rule**, ComfyUI
never returning its VRAM; and the **dead-wood routing rule**, the foliage guardrail.
...
It earned its keep on the first run. **The variants gate had been crashing for three releases** -- its stand-in
`UILayout` had no `column`, which the stylise block the stylise gate added to the panel body calls -- and it looked
...
than trusting an exit code. Nobody had re-run the variants gate in two phases, which is exactly the failure mode the
...
- `core/comfy.py` against a stdlib `http.server` fake: queue, jobs-API status shapes, cancel
  ...
  **Shipped by the first spike** as `tools/tests/test_comfy.py` (13 tests, no server contacted): the fake covers
  ...
  upstream template, and is not mutated by templating. **the variants gate took it to 33 tests**: a multipart
...
- `tools/scripts/comfy_preflight.py` over every shipped workflow against the real server,
  including the `BOB_*` title presence and uniqueness check (the title-template rule) and the **no-`api_node`
  assertion**, which is the test that keeps local-only true over time. **Shipped by the variants gate**; it also
...
- `core/comfy_maps.py` on synthetic albedo, and `core/heightfields` on a synthetic mask. **The macro
  half shipped by the macro-mask gate**: five tests in `test_comfy.py` (the mask is the low band and not the high one
...
- `core/comfy_maps.py` on synthetic albedo. **Shipped by the first spike and extended by the variants gate**: PNG round trip
  ...
  edge from continuous noise, and by the variants gate: roughness using the whole band on a bright albedo and
...
- `tools/scripts/headless_gen_variants_maps.py`, the variants gate: ten generate-and-accept cycles with the drift
  ...
  and after a `tex_upres` upres, roughness contrast the first spike against the variants gate on the same image, and the `load_post`
  reset. Reachability-gated for the half that needs a server; the preflight, maps and scheduler
  half always runs.
- Headless, texture sets: apply a fixture set to a terrain layer, assert image nodes are wired into the
  master's map sockets and the render is not flat. **the first spike added the generated-set counterpart**,
...
- Headless, mesh generation: import a real generated GLB, run simplify through bake, then assert face count
  ...
  `materials.master_type()` reports a BobShader. **Shipped by the asset gate** over five assets, and it caught
...
- Headless, mesh generation with open surfaces: the same over **non-manifold, single-sided geometry**. **Shipped
  by the asset gate**, and it is where the plan's open-surface assumption was corrected twice: the boundary-edge
  ...
  asserted (off, so a blade renders from behind); the opacity channel is measured and, since the route A/B,
  reports **wired** on the leaf.
- `tools/scripts/headless_gen_stylise_paint_multiview.py`, the stylise gate, in four parts (`--part a,b,c,d`) because they
...
- `tools/scripts/headless_gen_blockout_control.py`, the control gate, in four parts (`--part a,b,c,d`).
  ...
  **C**: one block-out all the way through `mesh_geom_ctrl`, `mesh_simplify_uv`, `mesh_texture` and steps 6 to 8, against the asset gate asset checks
...
- `tools/scripts/headless_gen_terrain_macro.py`, the macro-mask gate, in five parts (`--part a,b,c,d,e`).
...
- `tools/scripts/headless_gen_agent_surface.py`, the agent-surface gate, in four parts (`--part a,b,c,d`), and the one
...
- `tools/scripts/headless_gen_geometry_ab.py`, the geometry A/B gate, in four parts (`--part a,b,c,d`), inside
...
- `tools/scripts/headless_gen_bbox_control.py`, the bbox gate, in four parts (`--part a,b,c,d`), inside
  Blender. **A**: preflight over every shipped graph offline against the committed dump, the
  ...
  them, `mesh_geom_bbox`'s `auto_bbox` binding both ways, and the staged-copy fault's tripwire (the installed `comfy-aimdo` version
  against the one the agent-surface gate measured). No server, always runs, costs a second. **B**: the grid, the same
  three block-outs and the same conditioning image the control gate used, three control columns (`mesh_geom_ctrl` point, `mesh_geom_bbox`
...
- `tools/scripts/headless_gen_voxel_control.py`, the voxel gate, in four parts (`--part a,b,c,d`), inside
  Blender. **A**: preflight over every shipped graph offline against the committed dump, the
  ...
  `comfy.VOXEL_INPUT_ROTATION`, one exporter serving two modes, and the staged-copy fault's tripwire again. No server,
...
- `tools/scripts/headless_gen_oneshot_vs_staged.py`, the route A/B gate: ten prompts through both routes off ONE
