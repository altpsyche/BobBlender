...
This repo was once indexed by the phase that built each thing (`F1`-`F6`, `G0`-`G9`, `S1`-`S5`,
`C1`-`C5`, `P0`-`P7`, `D1`-`D16`, `R1`-`R21`, `W1`-`W14`, `A1`+, `B1`+, `M1`-`M2`) rather than by
what the thing is. Those labels have three problems and only the first is cosmetic: they are the
*only* index into the repo's knowledge, so a reader has to learn a private chronology before the
software; their shelf life expired the moment the phase shipped; and two of the letters mean two
different things each (`R` is both a splines polish pass and a generation review finding, `W` is
both a generation route and a water-shader look pass), so the index is ambiguous as well as
private. `docs/CONVENTIONS.md` states the replacement rule. This is its enforcement.
...
LABEL_RE = re.compile(
    ...
    r"|D1[0-9]|D[1-9]"   # generation decisions
    r"|R1[0-9]|R2[01]|R[1-9]"   # generation review findings AND splines polish, the collision
    r"|W1[0-4]|W[1-9]"   # generation routes AND the water look pass, the other collision
